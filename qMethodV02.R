...
ggarrange(
  ...
  ncol = 1,
  labels = c("Real boot strapped sample")) %>%
  ggexport(
    filename = "distances2FactorsReal.png",
    width = 1600,
    height = 800)
...
ggarrange(
  bootStrapped.scores, 
  ncol = 1,
  labels = c("Real boot strapped sample: Factors' scores")
  ) %>%
  ggexport(
    filename = "scores2FactorsReal.png",
    width = 800,
    height = 800)

## computing Factors' Eucleidian Distance
factEuDist = sqrt(sum((xy$factor1 - xy$factor2) ^ 2))
factEuDist
